...
\begin{pojmybox}
\textbf{Klíčové pojmy:}
pasivní vs. aktivní součástka, rezistor, termistor (NTC/PTC), varistor (VDR), barevný kód, kondenzátor, elektrolytický kondenzátor, cívka, frekvenční filtry (LP, HP, BP, BS), propust, zádrž, SMD montáž
\end{pojmybox}
...
Akumulují elektrický náboj v elektrostatickém poli.
\textbf{Hlavní typy:}
\begin{itemize}
...
Akumulují energii do magnetického pole (\textit{viz Okruh 3}).
\begin{itemize}
    \item \textbf{Vzduchové cívky:} Mají malou indukčnost, používají se pro vysoké frekvence (vysílače, rádia).
    \item \textbf{Cívky s feritovým jádrem:} Ferit výrazně zvyšuje indukčnost. Slouží často jako \textbf{tlumivky} – tlumí vysokofrekvenční rušení ve zdrojích.
\end{itemize}

% ================================================================
\subsection*{6. Frekvenční filtry a jejich chování v AC a DC obvodech}

Kombinací R, L a C obvodů tvoříme filtry. Využívá se faktu, že \textbf{odpor (reaktance) kondenzátoru pro vysoké frekvence klesá}, zatímco \textbf{reaktance cívky roste}. U stejnosměrného proudu (DC, $f=0\,\mathrm{Hz}$) se kondenzátor chová jako rozpojený obvod (nekonečný odpor) a cívka jako zkrat (kus drátu).

\begin{center}
\begin{tikzpicture}[>=Stealth, font=\small, thick]
  % Dolní propust
  \begin{scope}[xshift=0cm, yshift=5cm]
    \draw (0,2.5) to[short, o-] (0.5,2.5) to[R, l=$R$] (2.5,2.5) coordinate (mid) to[short, -o] (3.5,2.5);
    \draw (0,0) to[short, o-o] (3.5,0);
    \draw (mid) to[C, l=$C$, *-*] (mid |- 0,0);
    \node[above] at (1.75, 3) {\textbf{Dolní propust (Low-Pass)}};
    % Graf
    \draw[->] (4.5,0) -- (6.5,0) node[right] {$f$};
    \draw[->] (4.5,0) -- (4.5,2.7) node[above] {$A_u$};
    \draw[accent, line width=1.5pt] (4.5,2.2) to[out=0, in=150] (6.2,0.2);
  \end{scope}

  % Horní propust
  \begin{scope}[xshift=8cm, yshift=5cm]
    \draw (0,2.5) to[short, o-] (0.5,2.5) to[C, l=$C$] (2.5,2.5) coordinate (mid) to[short, -o] (3.5,2.5);
    \draw (0,0) to[short, o-o] (3.5,0);
    \draw (mid) to[R, l=$R$, *-*] (mid |- 0,0);
    \node[above] at (1.75, 3) {\textbf{Horní propust (High-Pass)}};
    % Graf
    \draw[->] (4.5,0) -- (6.5,0) node[right] {$f$};
    \draw[->] (4.5,0) -- (4.5,2.7) node[above] {$A_u$};
    \draw[accent, line width=1.5pt] (4.5,0.2) to[out=30, in=180] (6.2,2.2);
  \end{scope}

  % Pásmová propust
  \begin{scope}[xshift=0cm, yshift=0cm]
    \draw (0,2.5) to[short, o-] (0.5,2.5) to[C, l=$C_1$] (2,2.5) to[R, l=$R_1$] (3.5,2.5) coordinate (mid) to[short, -o] (4.5,2.5);
    \draw (0,0) to[short, o-o] (4.5,0);
    \draw (mid) to[R, l=$R_2$, *-] (mid |- 0,1.25) to[C, l=$C_2$, -*] (mid |- 0,0);
    \node[above] at (2.25, 3) {\textbf{Pásmová propust (Band-Pass)}};
    % Graf
    \draw[->] (5.5,0) -- (7.5,0) node[right] {$f$};
    \draw[->] (5.5,0) -- (5.5,2.7) node[above] {$A_u$};
    \draw[accent, line width=1.5pt] (5.5,0.2) to[out=70, in=180] (6.5,2.2) to[out=0, in=110] (7.3,0.2);
  \end{scope}

  % Pásmová zádrž
  \begin{scope}[xshift=8cm, yshift=0cm]
    \draw (0,2.5) to[short, o-] (0.5,2.5);
    \draw (0.5, 3.2) to[L, l=$L$] (2.5, 3.2);
    \draw (0.5, 1.8) to[C, l_=$C$] (2.5, 1.8);
    \draw (0.5,2.5) -- (0.5,3.2); \draw (0.5,2.5) -- (0.5,1.8);
    \draw (2.5,2.5) -- (2.5,3.2); \draw (2.5,2.5) -- (2.5,1.8);
    \draw (2.5,2.5) coordinate (mid) to[short, -o] (3.5,2.5);
    \draw (0,0) to[short, o-o] (3.5,0);
    \draw (mid) to[R, l=$R$, *-*] (mid |- 0,0);
    \node[above] at (1.75, 3.5) {\textbf{Pásmová zádrž (Notch)}};
    % Graf
    \draw[->] (4.5,0) -- (6.5,0) node[right] {$f$};
    \draw[->] (4.5,0) -- (4.5,2.7) node[above] {$A_u$};
    \draw[accent, line width=1.5pt] (4.5,2.2) to[out=0, in=110] (5.5,0.2) to[out=70, in=180] (6.3,2.2);
  \end{scope}
\end{tikzpicture}
\end{center}

\textbf{Přehled propustnosti na různých frekvencích (DC vs AC):}

\begin{center}
\begin{tabularx}{\linewidth}{@{} l X X X @{}}
\toprule
\textbf{Typ filtru} & \textbf{Chování na DC ($0\,\mathrm{Hz}$)} & \textbf{Nízké frekvence (AC)} & \textbf{Vysoké frekvence (AC)} \\
\midrule
\textbf{Dolní propust} & \textcolor{green!60!black}{\textbf{Propouští plně}} (Kondenzátor C je rozpojen) & Propouští & \textcolor{red}{\textbf{Blokuje}} (Signál se zkratuje přes C na zem) \\
\midrule
\textbf{Horní propust} & \textcolor{red}{\textbf{Blokuje}} (Sériový C zablokuje SS proud) & \textcolor{red}{\textbf{Blokuje}} & \textcolor{green!60!black}{\textbf{Propouští plně}} \\
\midrule
\textbf{Pásmová propust} & \textcolor{red}{\textbf{Blokuje}} (Zastaví jej C1) & \textcolor{red}{\textbf{Blokuje}} & \textcolor{red}{\textbf{Blokuje}} (Zkratuje jej C2) \\
\midrule
\textbf{Pásmová zádrž} & \textcolor{green!60!black}{\textbf{Propouští plně}} & \textcolor{green!60!black}{\textbf{Propouští}} & \textcolor{green!60!black}{\textbf{Propouští}} (Zablokuje jen střední frekvenci) \\
\bottomrule
\end{tabularx}
\end{center}
